...
const UpdateCar = () => {
  ...
  useEffect(() => {
    fetch(API_URL + 'api/details-agency/' + params.id)
      .then(response => response.json())
      .then(result => {
        console.log(result);
        setAgency(result.agency[0]);
        setName(result.agency[0].name);
        setAddress(result.agency[0].address);
        setPhone(result.agency[0].phone);
        setPicture(result.agency[0].picture);
        setEmail(result.agency[0].email);
        setCity(result.agency[0].city.name);
        setPrice(result.agency[0].price);
        setLink(result.agency[0].link);
        setLongitude(result.agency[0].longitude);
        setLatitude(result.agency[0].latitude);
        setDescription(result.agency[0].description);
  ...
  const submit = async (e) => {
    ...
    try {
      const response = await axios.post(API_URL + 'api/update-agency/' + params.id, formData);
